...
// FIX: Removed .js extensions from imports to allow module resolver to find appropriate files (.ts, .tsx, .js).
...
function handleApplicationSubmit(e) {
    e.preventDefault();
    // FIX: Cast e.target to HTMLFormElement for FormData constructor.
...
function handlePostMission(e) {
    ...
    db.addMission({
        ...data,
        clientId: client.id,
        // FIX: Cast FormDataEntryValue to string for Date, parseFloat, and parseInt.
        startTime: new Date(data.startTime as string),
        endTime: new Date(data.endTime as string),
        payRate: parseFloat(data.payRate as string),
        requiredGuards: parseInt(data.requiredGuards as string, 10),
        requiredLevel: parseInt(data.requiredLevel as string, 10),
    });
...
function handleCreatePayroll(e) {
    ...
    // FIX: Cast e.target to HTMLFormElement for FormData constructor.
    const formData = new FormData(e.target as HTMLFormElement);
    // FIX: Cast FormDataEntryValue to string for Date constructor.
...
function handleContractSubmit(e) {
    ...
    const data = Object.fromEntries(formData.entries());
     db.addContract({
        ...data,
        // FIX: Cast FormDataEntryValue to string for Date and parseFloat.
        startDate: new Date(data.startDate as string),
        endDate: new Date(data.endDate as string),
        totalBudget: parseFloat(data.totalBudget as string),
    });
...
function initializeApp() {
    ...
    document.body.addEventListener('click', (e) => {
        ...
        const actions = {
            ...
            'update-roster': () => handleUpdateRoster((target as HTMLElement).dataset.guardId, (target as HTMLElement).dataset.listType),
