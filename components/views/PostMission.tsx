...
export const PostMission = ({ user }) => {
    ...
    if (!client) return `<div class="text-center p-8 bg-[var(--color-bg-surface)] rounded-lg shadow-sm border border-[var(--color-border)]">Your client profile could not be loaded. Please contact support.</div>`;
    ...
    return `
        <div class="animate-in max-w-4xl mx-auto" style="opacity: 0;">
            <h1 class="text-3xl font-bold text-[var(--color-text-base)] mb-6">Post a New Mission</h1>
            <form id="post-mission-form" class="bg-[var(--color-bg-surface)] p-8 border border-[var(--color-border)] rounded-lg shadow-md space-y-6">
                ...
                <div class="text-right"><button type="submit" class="inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-[var(--color-secondary-text)] bg-[var(--color-secondary)] hover:bg-[var(--color-secondary-hover)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[var(--color-accent)]">${Icons.PlusCircle({ className: "w-5 h-5 mr-2" })} Post Mission</button></div>
            </form>
        </div>
    `;
};