...
export const ContractModal = ({ user }) => {
    ...
    <div class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4 animate-in" data-action="close-modal-backdrop">
        <div class="bg-[var(--color-bg-surface)] rounded-lg shadow-xl w-full max-w-lg border border-[var(--color-border)] max-h-[90vh] overflow-y-auto" data-modal-content>
            <div class="p-6">
                <h2 class="text-2xl font-bold mb-4 text-[var(--color-text-base)]">Create New Contract</h2>
                <form id="contract-form" class="space-y-4">
                    ...
                    <div class="pt-4 border-t border-[var(--color-border)]">
                        <label class="flex items-center">
                            <input type="checkbox" name="addSite" id="add-site-checkbox" class="h-4 w-4 text-[var(--color-accent)] border-[var(--color-border)] rounded focus:ring-[var(--color-accent)]" />
                            <span class="ml-2 text-sm font-medium text-[var(--color-text-muted)]">Add a new site with this contract</span>
                        </label>
                        <div id="new-site-fields" class="hidden mt-4 space-y-4">
                             <div>
                                <label class="block text-sm font-medium text-[var(--color-text-muted)]">New Site Name</label>
                                <input name="siteName" placeholder="e.g., West Campus Building" class="${inputStyles}" />
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-[var(--color-text-muted)]">New Site Address</label>
                     ...
                     <div class="flex justify-end space-x-3 pt-4">
                         ...
                         <button type="submit" class="px-4 py-2 bg-[var(--color-secondary)] text-[var(--color-secondary-text)] font-semibold rounded-md hover:bg-[var(--color-secondary-hover)] transition">Submit for Approval</button>
                    </div>
                </form>
            </div>
        </div>
